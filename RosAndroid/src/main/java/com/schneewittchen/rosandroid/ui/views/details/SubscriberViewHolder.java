...
class SubscriberViewHolder implements IBaseViewHolder {
    ...
    public void baseBindEntity(BaseEntity entity) {
        this.entity = entity;
        String topicName = entity.topic.getName();
        String messageType = entity.topic.getType();
    ...
    public void baseUpdateEntity(BaseEntity entity) {
        entity.topic.setName(topicNameTextView.getText().toString());
        entity.topic.setType(topicTypeEditText.getText().toString());
    }
    ...
    private void selectNameItem(int position) {
        ...
        for (Topic rosTopic : availableTopics) {
            if (rosTopic.getName().equals(selectedName)) {
                topicTypeEditText.setText(rosTopic.getType());
            }
        }
    ...
    private void updateTopicNameSpinner() {
        ...
        for (Topic rosTopic : availableTopics) {
            if (topicTypes.isEmpty()) {
                topicNameItemList.add(rosTopic.getName());
            }

            for (String topicType : topicTypes) {
                if (rosTopic.getType().equals(topicType)) {
                    topicNameItemList.add(rosTopic.getName());
                    break;
                }
            }
        ...
        if (topicNameItemList.isEmpty()) {
            topicNameItemList.add(entity.topic.getName());
        } else {
